...
export default function PreviewModal({
  ...
  const feedSwiper = (
    ...
    >
      {groups.map((g, idx) => (
        <SwiperSlide key={g.id || `${g.theme || 'group'}-${idx}`}>
          {idx === groupIdx ? (
            <div className="modalBody">{renderMedia(g, clampedIdx, true)}</div>
          ) : (
            <div className="modalBody inactive">{renderInactiveSlide(g)}</div>
          )}
        </SwiperSlide>
      ))}
    </Swiper>
    ...
    <div className={`modal ${feedMode ? 'feed' : ''}`} role="dialog" aria-modal="true" aria-label="预览">
      ...
      <div className="modalPanel">
        ...
        <div className="modalBody">
          ...
          {feedMode && !Array.isArray(feedSwiper) && (
            <div className="feedOverlay">
              <div className="feedTitle">
              ...
              <div className="feedSub">
                {escHtml(
                  `${group.timeText || ''} | ${group.groupType || ''} | ${clampedIdx + 1}/${items.length} | 上下滑切换`
                )}
              </div>
            </div>
          )}
