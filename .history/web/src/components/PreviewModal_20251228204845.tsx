...
export default function PreviewModal({
      ...
      if (isActive) {
        mediaElement = (
          <>
            <video
              ref={videoRef}
              key={`${targetGroup.id || 'g'}-${currentIdx}`}
              src={media.url}
              controls
              autoPlay
              playsInline
  ...
  const feedSwiper = (
    ...
    >
      {groups.map((g, idx) => (
        <SwiperSlide key={g.id || `group-${idx}`}>
          {idx === groupIdx ? (
            <div className="feedSlide active">{renderMedia(g, clampedIdx, true)}</div>
          ) : (
