...
function makeThumb(item, groupIdx, itemIdx) {
  ...
  const mediaEl = isVideo
    ? `<video preload="metadata" muted playsinline src="${escHtml(item.url)}"></video>`
...
function render() {
  ...
  meta.textContent = `groups: ${showGroups}/${totalGroups}  |  items: ${showItems}/${totalItems}  |  filter: ${state.activeType}  |  q: ${state.q || "-"}`;
